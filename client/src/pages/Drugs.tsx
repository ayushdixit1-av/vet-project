...
const categoryOptions = ['All', 'Antibiotic', 'Antiparasitic', 'Anti-inflammatory', 'Anesthetic', 'Vaccine', 'Supplement', 'Other']
...
export default function Drugs() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['medicines', search, species, category, page],
    queryFn: () => {
  ...
  const drugs = Array.isArray(data) ? data : (data as any)?.data ?? []
  const totalPages = (data as any)?.last_page ?? Math.ceil(((data as any)?.total ?? drugs.length) / limit)

  if (error) return (
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="text-center">
        <p className="text-red-400 text-lg">Failed to load drugs</p>
    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
          <div className="text-center">
            <Pill className="w-12 h-12 mx-auto text-emerald-400 mb-4" />
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Drug Database</h1>
            <p className="text-lg text-emerald-100/80 max-w-2xl mx-auto">
              Comprehensive veterinary pharmaceutical reference database
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <Pill className="w-16 h-16 mx-auto text-gray-600 mb-4" />
            <p className="text-gray-400 text-lg">No drugs found</p>
