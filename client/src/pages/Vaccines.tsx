...
import { Search, Syringe, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function Vaccines() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['vaccines', search, species, page],
    queryFn: () => {
      let url = `/vaccines?page=${page}&limit=${limit}`
  ...
  const vaccines = Array.isArray(data) ? data : (data as any)?.data ?? []
  const totalPages = (data as any)?.last_page ?? Math.ceil(((data as any)?.total ?? vaccines.length) / limit)

  if (error) return (
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="text-center">
        <p className="text-red-400 text-lg">Failed to load vaccines</p>
    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
          <div className="text-center">
            <Syringe className="w-12 h-12 mx-auto text-emerald-400 mb-4" />
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Vaccines</h1>
            <p className="text-lg text-emerald-100/80 max-w-2xl mx-auto">
              Veterinary vaccine information, schedules, and recommendations
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(vaccines as Vaccine[]).map((vaccine) => (
              <Link
                key={vaccine.id}
                to={`/vaccines/${vaccine.slug}`}
              ...
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-sm text-gray-400 line-clamp-2 mb-3">{vaccine.description}</p>
                  {vaccine.schedule && (
                    <p className="text-xs text-gray-500"><span className="text-gray-400">Schedule:</span> {vaccine.schedule}</p>
                  )}
                </div>
              </Link>
            ))}
          ...
          <div className="text-center py-20">
            <Syringe className="w-16 h-16 mx-auto text-gray-600 mb-4" />
            <p className="text-gray-400 text-lg">No vaccines found</p>
